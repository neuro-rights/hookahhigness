...
def create_item():
    ...
    for index in range(st.session_state.attrs):
        ...
        attr_json = {}
        attr_type = st.selectbox("Attribute type", attr_types, key=f"attr_types_index_{index}")
        ...
        if attr_type == "Number":
            ...
            display_type = st.selectbox("dislay type", display_types, key=f"display_index_{index}")
...
def mint() -> None:
    ...
    st.write(f'{contracts[contract_name]["token_name"]}  - {contracts[contract_name]["token_symbol"]} ')
    st.write(f"Contract address: {contract}")
